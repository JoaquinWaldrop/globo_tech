...
class TabletController extends AweController
{
	/**
	...
	public function actionCreate()
	{
        ...
        $this->performAjaxValidation($model, 'tablet-form');

        if(isset($_POST['Tablet']))
		{
			$model->attributes = $_POST['Tablet'];
            ...
            if ((is_object($file_imagen1) && get_class($file_imagen1)==='CUploadedFile')){
                ...
                if(!file_exists(Yii::app()->basePath.'/../img/tablet'))
                    mkdir(Yii::app()->basePath.'/../img/tablet', 0777, true);

                $nombre1=mt_rand(0,10000).$file_imagen1->getName();
                $model->imagen = 'img/tablet/'.$nombre1;
                }
            else
            {
            ...
            if($model->validate()) $file_imagen1->saveAs(Yii::app()->basePath.'/../img/tablet/'.$nombre1);
			if($model->save()) {
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['Tablet']))
		{
            ...
            if ((is_object($file_imagen) && get_class($file_imagen)==='CUploadedFile')){
                ...
                $model->imagen = 'img/tablet/'.$nombre;
                $file_imagen->saveAs(Yii::app()->basePath.'/../img/tablet/'.$nombre);
                }
            else
            {
            	Yii::app()->user->setFlash('error', "Formato de imagen no valido");
	...
	{
		$model = new Tablet('search');
		$model->unsetAttributes(); // clear any default values
		if(isset($_GET['Tablet']))
			$model->attributes = $_GET['Tablet'];
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax'] === 'tablet-form')
		{
