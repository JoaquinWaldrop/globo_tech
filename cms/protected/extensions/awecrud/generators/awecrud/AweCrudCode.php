    ...
    public function generateActiveField($modelClass, $column)
    {
        ...
        if (strtoupper($column->dbType) == 'TINYINT(1)'
            ...
            if (strtoupper($column->dbType) == 'DATETIME') {
                ...
                'attribute'=>'diadeSalida', //attribute name
                'mode'=>'date', //use "time","date" or "datetime" (default)
                'options'=>array("dateFormat"=>'yy-mm-dd'),// jquery plugin options
                    ));

                <?php echo $form->datepickerRow($model, 'fecha_ingreso', array('prepend'=>'<i class="icon-calendar"></i>')) ?>
                 */
                /*return "\$form->datepickerRow(\$model, '{$column->name}', array('prepend'=>'<i class=\"icon-calendar\"></i>'))";*/
                return "\$this->widget('CJuiDateTimePicker',array('model'=>\$model,'attribute'=>'{$column->name}','mode'=>'datetime','language'=>Yii::app()->language,'options'=>array('regional'=>Yii::app()->language,'dateFormat'=>'yy-mm-dd h:i:s')));";
            } else {
                if (strtoupper($column->dbType) == 'DATE') {
                    ...
                    <?php echo $form->datepickerRow($model, 'fecha_ingreso', array('prepend'=>'<i class="icon-calendar"></i>')) ?>'language'=>'',
                    'options'=>array(
                            'regional'=>'',
                        )
                     */
                    /*return "\$form->datepickerRow(\$model, '{$column->name}', array('prepend'=>'<i class=\"icon-calendar\"></i>'))";*/
                    return "\$this->widget('CJuiDateTimePicker',array('model'=>\$model,'attribute'=>'{$column->name}','mode'=>'date','language'=>Yii::app()->language,'options'=>array('regional'=>Yii::app()->language,'dateFormat'=>'yy-mm-dd')));";
                } else {
    ...
    public function generateGridViewColumn($modelClass, $column)
    {
        ...
        } else // Common column.
        {
            if (strtoupper($column->dbType) == 'DATE'
                ...
                || strtoupper($column->dbType) == 'TIMESTAMP'
            ) {
                ...
                return "array(
                        ...
                        'filter' => \$this->widget('zii.widgets.jui.CJuiDatePicker', array(
                                ...
                                'htmlOptions' => array(
                                    'id' => 'picker_{$column->name}',
                                    'style'=>'width:100px',
                                    'onChange' => 'jQuery(\"#picker_{$column->name}\").datepicker(\"hide\")'
                                ),
